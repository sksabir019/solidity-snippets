...
contract DuringConstruction {
    constructor(ExistingContract someExistingContract) payable {
        ...
        // 2: CAN OTHER CONTRACTS CALL MY FUNCTIONS? MAKE ANOTHER EXISTING CONTRACT CALL MY FUNCTION DURING CONSTRUCTION - err
        // require(someExistingContract.someFunction(this) == 4);

        // 3: CAN I CALL MY OWN EXTERNAL FUNCTIONS? CALL MY OWN FUNCTION, BUT AS AN EXTERNAL FUNCTION: - err
        // SO IT'S LIKE IM SEEING MYSELF FROM THE PERSPECTIVE OF OUTSIDE MYSELF
        // require(this.someExternalFunction() == 6);

        // 4: MAKE THE EXISTING CONTRACT SEND ETHER TO US - DOES IT TRIGGER THE FALLBACK?: - yes 
        someExistingContract.sendFundsToAContractDuringItsConstruction{
            value: 1 ether
        }(this);
    }
...
contract ExistingContract {
    ...
    function sendFundsToAContractDuringItsConstruction(
        DuringConstruction _contract
    ) external payable {
